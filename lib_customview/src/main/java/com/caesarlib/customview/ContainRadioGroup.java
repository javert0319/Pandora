...
/**我修改过的自定义流式布局，原先的里面只能放radiogroup，这个里面可以放view
 * Created by Dengxiao on 2016/12/2.
 */

public class ContainRadioGroup extends RadioGroup {

    private List<RowView> rowViews;//存放每行radioBotton的集合
    private int horizontalSpacing = 60;//默认水平间距
    private int verticalSpacing = 30;//默认垂直间距
    ...
    public ContainRadioGroup(Context context) {
        this(context, null);
    }

    public ContainRadioGroup(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            } else if (rowView.getRowWidth() + horizontalSpacing + childView.getMeasuredWidth() >
                    nopaddingWidth) {
        ...
        for (int i = 0; i < rowViews.size(); i++) {
            ...
            for (int j = 0; j < viewList.size(); j++) {
                ...
                if (j == 0) {
                    ...
                    childView.layout(left, preView.getTop(), left + childView.getMeasuredWidth(), preView
                            .getBottom());
                }
            }
        }
